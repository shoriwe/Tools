...
def better_copytree(src, dst, symlinks=False, ignore=None):
...
def cd(sender):
    if sender.title == 'Back':
        ...
        view.data_source.update_items(better_join(current_dir, '..'))
    else:
        ...
        current_dir = view.data_source.current_dir
        view.data_source.update_items(better_join(current_dir, item))
    # Always reload the view (this updates the tableview values)
    view.reload()


# Quicker and cleanest way to create a button
def button_creator(title, action, frame, color):
    button = ui.Button()
    button.title = title
...
class CustomDataSource(object):
    def __init__(self, pwd=get_home()):
        ...
        self.button_cell_frame_ref = (
            (screen_size[0] * 320) / 414, (cell_frame[3] * 15) / 44, (cell_frame[2] * 100) / 414,
            (cell_frame[3] * 30) / 44)

    # Update the values of the items with the content of the current directory
    def update_items(self, pwd):
        self.items = list(filter(lambda file: isdir(better_join(pwd, file)), listdir(pwd)))
        # update  the new current directory
        self.current_dir = pwd

    #  Creation of the button for copy files
    def copyhere_button(self):
        button = button_creator('Copy here', self.copy, self.button_cell_frame_ref, '#80f228')
        return button
    ...
    def tableview_cell_for_row(self, tableview, section, row):
        ...
        button = self.copyhere_button()
        # Check if the button was disabled before
        if better_join(self.current_dir, self.items[row]) in self.disabled_buttons:
